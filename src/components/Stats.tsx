...
function useBenefits(): Benefit[] {
  const pct = useCountUp(94, { suffix: '%', duration: 1400 })
  const hrs = useCountUp(24, { suffix: '시간', duration: 1000 })
  const yrs = useCountUp(5, { suffix: '년', duration: 1000 })
  const rank = useCountUp(1, { suffix: '위', duration: 800 })
  const won = useCountUp(100, { suffix: '만원', duration: 1200 })
  const cards = useCountUp(9, { suffix: '개사+', duration: 1000 })

  return [
    {
      title: <span ref={pct.ref}>{pct.display}</span>,
      sub: '업계 최고 지급률',
    ...
    {
      title: <><span ref={hrs.ref}>{hrs.display}</span> · 365일</>,
      sub: '연중무휴 상담',
    ...
    {
      title: <><span ref={yrs.ref}>{yrs.display}</span> 무사고</>,
      sub: '안전·신속 운영',
    ...
    {
      title: <><span ref={rank.ref}>{rank.display}</span> 업계</>,
      sub: '재이용률·후기 수',
    ...
    {
      title: <>최대 <span ref={won.ref}>{won.display}</span></>,
      sub: '핸드폰 소액 현금화',
    ...
    {
      title: <><span ref={cards.ref}>{cards.display}</span> 카드사</>,
      sub: '국내 전 카드사',
          ...
          {benefits.map((b, i) => (
            ...
            >
              ...
              <div className="relative">
                <div className="text-[20px] md:text-[26px] font-black text-[--text] leading-tight mb-1 break-keep">{b.title}</div>
